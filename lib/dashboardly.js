...
class DashboardlyDataLoader {
  ...
  getUserFromSession(sessionToken) {
    ...
    .then((result) => {
      ...
      if (result.length === 1) {
        let user = result[0];
        let result2 = util.generateHash(user.users_email);
        user.avatarUrl = `https://www.gravatar.com/avatar/${result2}?d=retro`;
        return user;
        // return bcrypt.hash(user.users_email.lowerCase(), 1)
        //   .then( (hash) => {
  ...
  createBookmark(bookmarkData) {
    // console.log('res', bookmarkData, util.filterKeys(BOOKMARK_WRITE_FIELDS, bookmarkData));
    ...
    return this.query(
      ...
      .then((result) => {
        // console.log('res', bookmarkData, result, util.filterKeys(BOOKMARK_WRITE_FIELDS, bookmarkData))
        return this.query(
          knex
            .select(BOOKMARK_FIELDS)
            .from('bookmarks')
